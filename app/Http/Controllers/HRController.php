...
class HRController extends Controller
{
    ...
    public function getIndexInitial2()
    {
        ...
        $select = Initial_leave::JoinUsers()->JoinDeptCategory()->select([
            ...
            ->whereYear('initial_leave.created_at', date('Y'))
    ...
    public function getIndexLeaveEntitled()
    {
		...
		->where('all_leave_entitled.name', '!=', 'Administrator')
		->where('all_leave_entitled.name', '!=', 'HR Verification')
		->where('all_leave_entitled.name', '!=', 'WIS Verification')*/
    ...
    public function storeTempCreateInitialLeave(Request $request)
    {
        $rule = [
        ...
        if ($validator->fails()) {
            ...
            if ($request->input('category') == 1) {
                if ($remainsAnnual > $request->input('request_day')) {
                    Session::flash('getError', Lang::get('messages.data_custom', ['data' => 'Sorry, annaul not enough!']));
                    return Redirect::route('hr_mgmt-data/leave/tempCreateInitialLeave', [$id]);
                }
            }
    ...
    public function insertForfeitedCounts($id)
    {
        ...
        if ($forfeited !== 0) {
            ...
            if ($remainsForfeited < 0) {
                $remainsForfeited = 0;
            }

            if ($remainsForfeited > $leave->total_day) {
                $amount = $leave->total_day;
            } else {
                $amount = $remainsForfeited;
            }
