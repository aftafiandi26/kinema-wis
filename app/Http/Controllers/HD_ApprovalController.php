...
class HD_ApprovalController extends Controller
{
  ...
  public function approveLeave(Request $request, $id)
  {
    ...
    if (auth::user()->dept_category_id === 7 and auth::user()->hd === 1) {
      ...
      // Mail::send('email.Notifikasi.Leave.verByMike', ['email' => $email, 'john' => $john], function($message) use ($email, $john)
      //    {
      //        $message->to($email->email, 'WIS')->subject('[Approved] Leave Application - by '.$john->first_name.''.$john->last_name.'');
      //        $message->from('wis_system@frameworks-studios.com', 'WIS');
      //    });
    } else {
