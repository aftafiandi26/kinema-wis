...
class HRDLevelAccess extends Controller
{
    ...
    {
        $this->middleware(['auth', 'active', 'level_hrd']);
    }
// Rusun 
...
public function exportProject()
 ...
 if ($request->file('photo') === NULL) {
    ...
    public function indexAllSummary()
    {
       ...
       */
        return view::make('HRDLevelAcces.summary.allSummary.index', 
            [
    ...
    public function getEndEmployee()
    {
     ...
     $select = NewUser::joinDeptCategory()->JoinLeaveView()->select(['users.id', 'users.nik', 'users.end_date', 'users.join_date', DB::raw("CONCAT(users.first_name, ' ', users.last_name) as fullname"), 'users.position', 'dept_category.dept_category_name', 'all_leave_entitled.annual_leave_balance',
