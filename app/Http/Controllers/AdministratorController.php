...
class AdministratorController extends Controller
{
    ...
    public function HeadOFDepartmentAccess()
    {
        return view('admin.HDLevelAccess.index');
    }

    public function dataHeadOfDepartmentAccess()
    {
        $data = NewUser::joinDeptCategory()->select([
            'users.id', 'users.nik', 'users.first_name', 'users.last_name', 'users.position', 'users.join_date', 'users.dept_ApprovedHOD', 'users.dept_category_id',
        ...
        return Datatables::of($data)
            ...
            ->addColumn('status', '@If($dept_ApprovedHOD == 1) {{ "Approval" }} @else {{ null }} @endif')
            ->addColumn('actions', function (NewUser $user) {
                $edit = "<a class='btn btn-sm btn-warning' href=" . route('admin/head-of-deparment/access/update', $user->id) . " title='change approved head of department'><span class='fa fa-refresh fa-spin'></span></a>";
    ...
    public function addHeadOfDepartmentAccess($id)
    {
        $data = [
            'dept_ApprovedHOD' => 1
        ];
        ...
        if ($user->dept_ApprovedHOD === 0) {
            User::where('id', $user->id)->update(['dept_ApprovedHOD' => 1]);
        } else {
            User::where('id', $user->id)->update(['dept_ApprovedHOD' => 0]);
        }

        Session::flash('message', Lang::get('messages.data_custom', ['data' => $user->first_name . ' ' . $user->last_name . ' has been approved']));
        return Redirect::route('admin/head-of-department/access');
    }
}
