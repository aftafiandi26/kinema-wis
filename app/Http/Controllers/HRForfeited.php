...
class HRForfeited extends Controller
{
    ...
    public function dataIndexUser()
    {
        ...
        return Datatables::of($query)
            ...
            ->addColumn('availableForfeited', function (User $user) {
                $availableLeave = $this->availableLeave(($user->id));
            ...
            ->addColumn('advancedLeave', function (User $user) {
                ...
                if ($this->availableLeave($user->id) <= 0) {
                    $return = 0;
                } else {
                    $return = $remain - $this->availableLeave($user->id);
                }
