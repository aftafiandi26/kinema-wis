...
class GM_ApprovalController extends Controller {
    ...
    public function getIndexLeaveApproval()
    {
        ...
        // ->edit_column('ap_hrd', '@if($ap_hrd === 1){{"Verified"}} @elseif($ap_hrd === 0){{Waitting HRD}} @endif')
    ...
    public function approveLeave(Request $request, $id)
    {
        ...
        Mail::send('email.approvedMail', ['email' => $email], function($message) use ($email)
            {
                $message->to('hr.verification@frameworks-studios.com', 'WIS')->subject('[Approved] Leave Application - WIS');
